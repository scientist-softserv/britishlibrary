  ...
  module FileAvailabilityFaceting
    ...
    private
      ...
      def set_file_availability_for_faceting
        if ('open'.in? get_work_filesets_visibility) && official_link.present? && (doi_option_value_check? == true)

          self.file_availability = ['File available from this repository']

        elsif ('open'.in? get_work_filesets_visibility) && official_link.blank? && (doi_option_value_check? == false)

          self.file_availability = ['File available from this repository']

        elsif ('open'.in? get_work_filesets_visibility) && official_link.present? && (doi_option_value_check? == false)

          multiple_values

        elsif (get_work_filesets_visibility.any? { |status| status.in? ['authenticated', 'restricted'] } || get_work_filesets_visibility.blank?) && official_link.present? && (doi_option_value_check? == false)

          self.file_availability = ["External link (access may be restricted)"]

        elsif (get_work_filesets_visibility.any? { |status| status.in? ['authenticated', 'restricted'] } || get_work_filesets_visibility.blank?) && official_link.blank? && (doi_option_value_check? == true)

          self.file_availability = ['File not available']

        elsif (get_work_filesets_visibility.any? { |status| status.in? ['authenticated', 'restricted'] } || get_work_filesets_visibility.blank?) && official_link.present? && (doi_option_value_check? == true)

          self.file_availability = ['File not available']

        elsif (get_work_filesets_visibility.any? { |status| status.in? ['authenticated', 'restricted'] } || get_work_filesets_visibility.blank?) && official_link.blank? && (doi_option_value_check? == false)
      ...
      def doi_option_value_check?
        doi_options.in? ["Mint DOI:Registered", "Mint DOI:Findable"]
      end

      def multiple_values
        if file_availability.include? "File not available"
          file_availability.delete "File not available"
          self.file_availability = file_availability | ['External link (access may be restricted)', 'File available from this repository']
        else
          self.file_availability = file_availability | ['External link (access may be restricted)', 'File available from this repository']
        end
      end
